...
def parse_rawdata():
    ...
    df.to_pickle(f'{root}/testA/totalExposureLog.pkl')
    del df
    gc.collect()
    ##############################################################################
    # 静态广告
    df = pd.read_csv(
        f'{root}/testA/ad_static_feature.out', sep='\t',
        names=['aid', 'create_timestamp', 'advertiser', 'good_id', 'good_type', 'ad_type_id',
               'ad_size'],

    ).sort_values(by='create_timestamp')
    df = df.fillna(-1)
    for f in ['aid', 'create_timestamp', 'advertiser', 'good_id', 'good_type', 'ad_type_id']: # good_id  ad_type_id
        items = []
    ...
    df.to_pickle(f'{root}/testA/ad_static_feature.pkl')
    del df
    gc.collect()
    ##############################################################################
    # 用户信息
    df = pd.read_csv(
        f'{root}/testA/user_data', sep='\t',
    ...
    df.to_pickle(f'{root}/testA/user_data.pkl')
    del df
    gc.collect()
    ##############################################################################
    # 测试数据
    df = pd.read_csv(
        f'{root}/testA/test_sample.dat', sep='\t',
    ...
    df.to_pickle(f'{root}/testA/test_sample.pkl')
    del df
    gc.collect()
...
def construct_log():
    ...
    train_df = pd.read_pickle(f'{root}/testA/totalExposureLog.pkl')
    ...
    log[log['request_day'] < 17944].to_pickle(f'{root}/user_log_dev.pkl')
    log.to_pickle(f'{root}/user_log_test.pkl')  # 看不懂
    del log
    del tmp
    gc.collect()
...
def extract_setting():
    aids = []
    with open(f'{root}/testA/ad_operation.dat', 'r') as f:
        for line in f:
...
train_dev_df.to_pickle(f'{root}/train_dev.pkl')
train_df.to_pickle(f'{root}/train.pkl')
dev_df.to_pickle(f'{root}/dev.pkl')
test_df.to_pickle(f'{root}/test.pkl')
